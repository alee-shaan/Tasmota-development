...
#define MAX_EMPLOYEE_LENGTH 10
#define MAX_INITIALS_LENGTH 2
...
void StartWebserver(int type)
{
  ...
  if (!Web.state) {
    if (!Webserver) {
      ...
      // Add a new handler for the specific request URL
      Webserver->on("/hub/networks/wi", HTTP_GET, HandleCustomRequest);
...
// New function to handle the custom request URL
void HandleCustomRequest() {
  // Retrieve the query parameters from the request
  String s1 = Webserver->arg("s1");
  String p1 = Webserver->arg("p1");

  // Retrieve the headers from the request
  String referer = Webserver->header("Referer");

  // Process the parameters and headers or perform desired actions
  // For example, you can print them or save them to variables
  Serial.println("Value of s1: " + s1);
  Serial.println("Value of p1: " + p1);
  Serial.println("Referer: " + referer);

  // Send a response back to the client
  Webserver->send(200, "text/plain", "Custom request handled successfully");
}
...
void WebRestart(uint32_t type) {
  ...
  if (3 == type) {
    WSContentSend_P("setTimeout(function(){location.href='http://localhost:3000/hub/settings';},%d);",
...
void HandleRoot(void)
{
  ...
  if (CaptivePortal()) { 
    Webserver->send(200, "application/json", "{\"status\":\"Captive portal active\"}");
...
void HandleWifiConfiguration(void) {
  ...
  if (Webserver->hasArg(F("storeInfo"))){
    storeInfo myStore;
    WebGetArg(PSTR("storeName"), tmp, sizeof(tmp));
    strncpy(myStore.store_name, tmp, MAX_NAME_LENGTH);
    WebGetArg(PSTR("storePhoneNumber"), tmp, sizeof(tmp));
    strncpy(myStore.store_phone_number, tmp, MAX_NUMBER_LENGTH);
    WebGetArg(PSTR("storeUniqueId"), tmp, sizeof(tmp));
    strncpy(myStore.store_unique_id, tmp, MAX_ID_LENGTH);
    WebGetArg(PSTR("storeAddress"), tmp, sizeof(tmp));
    strncpy(myStore.store_address, tmp, MAX_ADDRESS_LENGTH);

    Webserver->send(200, "text/plain", "Store Added Successfully");
  }
  if ((Webserver->hasArg(F("employeeInfo")))){
    employeeInfo myEmployee;
    WebGetArg(PSTR("employeeName"), tmp, sizeof(tmp));
    strncpy(myEmployee.employee_name, tmp, MAX_NAME_LENGTH);
    WebGetArg(PSTR("employeeInitials"), tmp, sizeof(tmp));
    strncpy(myEmployee.employee_initials, tmp, MAX_INITIALS_LENGTH);
    WebGetArg(PSTR("employeeEmailAddress"), tmp, sizeof(tmp));
    strncpy(myEmployee.employee_emailaddress, tmp, MAX_ADDRESS_LENGTH);
    WebGetArg(PSTR("employeePhoneNumber"), tmp, sizeof(tmp));
    strncpy(myEmployee.employee_phonenumber, tmp, MAX_NUMBER_LENGTH);
    WebGetArg(PSTR("employeeType"), tmp, sizeof(tmp));
    strncpy(myEmployee.employee_type, tmp, MAX_EMPLOYEE_LENGTH);
    WebGetArg(PSTR("employeeEmailNotify"), tmp, sizeof(tmp));
    myEmployee.employee_emailnotify = (strcmp(tmp, "true") == 0);
    WebGetArg(PSTR("employeeSmsNotify"), tmp, sizeof(tmp));
    myEmployee.employee_smsnotify = (strcmp(tmp, "true") == 0);


    Webserver->send(200, "text/plain", "Employee Added Successfully");
  }
  if ((Webserver->hasArg(F("passwordInfo")))){
    passwordInfo myPassword;
    WebGetArg(PSTR("password"), tmp, sizeof(tmp));
    strncpy(myPassword.employee_password, tmp, MAX_NUMBER_LENGTH);
    WebGetArg(PSTR("password_time"), tmp, sizeof(tmp));
    strncpy(myPassword.employee_passwordtime, tmp, MAX_ADDRESS_LENGTH);

    Webserver->send(200, "text/plain", "Password Added Successfully");
  }
...
bool CaptivePortal(void)
{
  ...
  if ((WifiIsInManagerMode()) && !ValidIpAddress(Webserver->hostHeader().c_str())) {
    ...
    Webserver->sendHeader(F("Location"), String(F("http://localhost:3000/hub/storeInfo")) , true);
...
void HandleUserInfoConfiguration(void){
